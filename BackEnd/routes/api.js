...
router.post('/forgotPass', async function (req, res, next) {
  try {
    ...
    const mailOptions = {
      ...
      text: 'Có cái mật khẩu cũng quên: http://localhost:5173/changePass/' + resetToken
    };
...
//filter by cate and price
router.get('/products/filter/:categoryName/:price-:lastPrice', async (req, res, next) => {
  ...
  const productsCollection = db.collection('products');
  const products = await productsCollection.find({ category_id: categoryId, price: { $gte: price, $lte: lastPrice } }).toArray();
...
//get Bill by id_bill
router.get('/bill/:id_bill', async (req, res, next) => {
  const id_bill = req.params.id_bill;
  console.log(typeof id_bill);
  const db = await connectDb();
  const billsCollection = db.collection('bills');
  const bill = await billsCollection.findOne({ id_bill: id_bill });
...
//update bill status
router.put('/bill/:id_bill/updateStatus', async (req, res, next) => {
  const id_bill = req.params.id_bill;
  const status = req.body.status;
  const db = await connectDb();
  const billsCollection = db.collection('bills');
  const result = await billsCollection.updateOne({ id_bill: id_bill }, { $set: { status: status } });
...
//get Bill by email
router.get('/bills/:email', async (req, res, next) => {
  const email = req.params.email;
  const db = await connectDb();
  const billsCollection = db.collection('bills');
  const bill = await billsCollection.find({ customer_email: email }).toArray();
...
//add Bills
router.post('/bills/add', upload.single('img'), async (req, res, next) => {
  try {
    let db = await connectDb();
    let billsCollection = db.collection("bills");

    let { id_bill, customer_name, customer_address, customer_phone, customer_email, totalCartMoney, method, status, order_date, goods } = req.body;

    // let lastBills = await billsCollection.find().sort({ id: -1 }).limit(1).toArray();
    // let id = lastBills[0] ? lastBills[0].id + 1 : 1;
